        ...
        for (int i=0; i<(response.getJSONArray("rows")).length();i++) {
           // System.out.println("el"+i+": "+response.getJSONArray("rows").getJSONObject(i));
            ...
            for (int j=0; j<array.length();j++) {
                ...
                {dist_el = 10000000;
                time_el = 10000000;}
                    tracks[i][j] = new TrackParameters(dist_el,time_el,destin.getString(i).toString(),destin.get(j).toString(),origins[i],destionations[j]);
            }
        }
        PostmanTrack tmp = new PostmanTrack(tracks);
        tmp.Tracks();
        System.out.println();
        for (TrackParameters i:tmp.sorted_track(origins[0])) {
